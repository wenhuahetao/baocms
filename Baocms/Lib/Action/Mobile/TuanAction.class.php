        ...
        if ($code == 'wait') {
            ...
            if (D('Tuanorder')->save(array('order_id' => $order_id, 'status' => '-1'))) {
                ...
                for ($i = 0; $i < $order['num']; $i++) {
                    $local = $obj->getCode();
                    $insert = array('user_id' => $this->uid, 'shop_id' => $tuan['shop_id'], 'order_id' => $order['order_id'], 'tuan_id' => $order['tuan_id'], 'code' => $local, 'price' => 0, 'real_money' => 0, 'real_integral' => 0, 'fail_date' => $tuan['fail_date'], 'settlement_price' => 0, 'create_time' => NOW_TIME, 'create_ip' => $ip);
                    $codes[] = $local;
                    $obj->add($insert);
                }
                ...
                $notice_data = array('url' => 'http://' . $_SERVER['HTTP_HOST'] . '/mcenter/tuan/detail/order_id/' . $order_id . '.html', 'first' => '亲,您的订单创建成功!', 'remark' => '详情请登录-http://' . $_SERVER['HTTP_HOST'], 'amount' => round($order['need_pay'] / 100, 2) . '元', 'order' => $order_id, 'info' => $tuan['title']);
            ...
            if (empty($order['use_integral'])) {
                ...
                if (!empty($this->member['integral'])) {
                    ...
                    D('Userintegrallogs')->add(array('user_id' => $this->uid, 'integral' => -$used, 'intro' => '订单' . $order_id . '积分抵用', 'create_time' => NOW_TIME, 'create_ip' => get_client_ip()));
                    $order['use_integral'] = $used;
                    $order['need_pay'] = $order['total_price'] - $order['mobile_fan'] - $used;
                    //再次更新一
                    //$this->fengmiMsg($order['mobile_fan']);//有错误
                    D('Tuanorder')->save($order);
                }
            }
            $logs = D('Paymentlogs')->getLogsByOrderId('tuan', $order_id);
            if (empty($logs)) {
                $logs = array('type' => 'tuan', 'user_id' => $this->uid, 'order_id' => $order_id, 'code' => $code, 'need_pay' => $order['need_pay'], 'create_time' => NOW_TIME, 'create_ip' => get_client_ip(), 'is_paid' => 0);
