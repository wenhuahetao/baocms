...
class CommonAction extends Action {
    ...
    protected function _initialize() {
        ...
        $invite = ( integer )$this->_get( "invite" );
        if ( !empty( $invite ) )
        {
            cookie( "invite_id", $invite );
        }
        $tui_uid = ( integer )$this->_get( "tui_uid" );
        if ( $tui_uid && ( $goods_id = ( integer )$this->_get( "goods_id" ) ) )
        {
