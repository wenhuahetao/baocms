	...
	public function loading() {
      ...
      public function yes($order_id=0){
        ...
        public function del(){            
            ...
            if(!$f){
               $this->ajaxReturn(array('status'=>'error','msg'=>'错误'));
				...
				$DeliveryOrder = D('DeliveryOrder')->where(array('type_order_id' => $order_id,'type' =>1)) -> setField('closed',1);//更改状态  
		...
		public function qx(){            
            ...
            $o = D('EleOrder');
            $f = $o -> where('order_id ='.$order_id) -> find();  
			$DeliveryOrder = D('DeliveryOrder')->where(array('type_order_id' => $order_id,'type' =>1)) -> setField('closed',0);//从新放入抢单          
            if(!$f){
                $this->fengmiMsg('错误！');
            }else{               
                if ($f['user_id'] != $this->uid) {
