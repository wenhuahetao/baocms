...
class EleAction extends CommonAction
{
    ...
    public function order()
    {
        ...
        if ($order_id = D('Eleorder')->add(array('user_id' => $this->uid, 'shop_id' => $shop_id, 'total_price' => $total['money'], 'logistics' => $shop['logistics'], 'need_pay' => $total['need_pay'], 'num' => $total['num'], 'new_money' => (int) $total['new_money'], 'settlement_price' => $settlement_price, 'status' => 0, 'create_time' => NOW_TIME, 'create_ip' => get_client_ip(), 'is_pay' => 0, 'month' => $month))) {
    ...
    public function pay2()
    {
        ...
        if ($code == 'wait') {
            ...
            /*微信订单通知消息-开始*/
            $notice_data = array('url' => 'http://' . $_SERVER['HTTP_HOST'] . '/mcenter/eleorder/detail/order_id/' . $order_id . '.html', 'first' => '亲,您的订单创建成功!', 'remark' => '详情请登录-http://' . $_SERVER['HTTP_HOST'], 'amount' => round($order['need_pay'] / 100, 2) . '元', 'order' => $order_id, 'info' => $product_name);
            ...
            $notice_data = array(
				...
				'order' => $order_id, 'info' => $product_name
			);
            $notice_data = Wxmesg::notice($notice_data);
            Wxmesg::net($this->uid, 'OPENTM202297555', $notice_data);
            /*微信订单通知消息-结束*/
