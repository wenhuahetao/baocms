...
class MallAction extends CommonAction
{
    ...
    public function order_change()
    {
        ...
        foreach ($goods as $val) {
            ...
            $tprice += $price;
            $ordergoods = array('num' => $num[$val['goods_id']], 'goods_id' => $val['goods_id'], 'price' => $val['mall_price'], 'total_price' => $price, 'js_price' => $js_price, 'update_time' => NOW_TIME, 'update_ip' => $ip);
    ...
    public function order() {
        ...
        foreach ($goods as $val) {
            $price = $val['mall_price'] * $num[$val['goods_id']];
            $dan_price = $val['mall_price'] * $num[$val['goods_id']];
            //总费用
            $use_price = $val['use_integral'] * $num[$val['goods_id']];
            //积分抵现多少钱
            $need_pay = $dan_price - $use_price;
            //实际支付金额
            $js_price = $val['settlement_price'] * $num[$val['goods_id']];
            $tprice += $price;
    ...
    public function pay2()
    {
        ...
        if ($code == 'wait') {
            ...
            if (empty($logs)) {
                $logs = array('type' => 'goods', 'user_id' => $this->uid, 'order_id' => $order_id, 'code' => $code, 'need_pay' => $need_pay, 'create_time' => NOW_TIME, 'create_ip' => get_client_ip(), 'is_paid' => 0);
                $logs['log_id'] = D('Paymentlogs')->add($logs);
            } else {
    ...
    public function goods_mum($order_id){
        ...
        foreach ($ordergoods_ids as $k => $v) {
            ...
            if ($goods_num['num'] < $v['num']) {
				$this->baoSuccess('商品ID' . $v['goods_id'] . '库存不足无法付款',U('Pchome/mall/index'));;
            }
        }
        return false;
    }
