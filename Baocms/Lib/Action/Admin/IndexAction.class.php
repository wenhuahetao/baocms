...
class IndexAction extends CommonAction
{
	...
	$counts['totay_dianping_waimai'] = (int) D('Eledianping')->where(array('create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time))))->count();		
	 ...
	 //订座数据
	$counts['shop_ding'] = (int) D('Shop')->where(array('is_ding'=>1))->count();
	$counts['shop_ding_room'] = (int) D('Shopdingroom')->count();
	$counts['shop_ding_yuyue'] = (int) D('Shopdingyuyue')->count();
	$counts['shop_ding_yuyue_day'] = (int) D('Shopdingyuyue')->where(array('create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time))))->count();
	$counts['shop_ding_dianping'] = (int) D('Shopdingdianping')->count();
    $counts['shop_ding_dianping_day'] = (int) D('Shopdingdianping')->where(array('create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time))))->count(); 	
	 //物流配送
	$counts['delivery'] = (int) D('Delivery')->count();
	$counts['delivery_order'] = (int) D('DeliveryOrder')->count();
	$counts['delivery_order_day_qiang_1'] = (int) D('DeliveryOrder')->where(array('status' => 0))->count();
	$counts['delivery_order_day_qiang_2'] = (int) D('DeliveryOrder')->where(array('status' => 1))->count();
	$counts['delivery_order_day_qiang'] = $counts['delivery_order_day_qiang_1'] + $counts['delivery_order_day_qiang_2'];

	$counts['delivery_order_day_zhong'] = (int) D('DeliveryOrder')->where(array('update_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time)), 'status' => array('EQ', 2)))->count();
	$counts['delivery_order_day_ok'] = (int) D('DeliveryOrder')->where(array('update_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time)), 'status' => array('EQ', 8)))->count();
    ...
    $counts['dianping_waimai'] = (int) D('Eledianping')->count();
    $counts['dianping_tuan'] = (int) D('Tuandianping')->count();
    $counts['dianping_goods'] = (int) D('Goodsdianping')->count();
	//优惠劵数据
	...
	$counts['totay_post'] = (int) D('Post')->where(array('create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time))))->count();
    $counts['post_audit'] = (int) D('Post')->where(array('audit' => 0))->count();
	$counts['post_reply'] = (int) D('Postreply')->count();
	$counts['totay_post_reply'] = (int) D('Postreply')->where(array('create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time))))->count();
	$counts['totay_post_reply_audit'] = (int) D('Postreply')->where(array('audit' => 0))->count();
    ...
    //社区村镇数据
	$counts['village'] = (int) D('Village')->count();
	$counts['village_bbs'] = (int) D('Village_bbs')->count();
	$counts['village_reply'] = (int) D('Villagebbsreplys')->count();
	$counts['village_suggestion'] = (int) D('Villagenotice')->count();
	$counts['villagejoin'] = (int) D('Villagejoin')->count();
	...
	$counts['article_zan'] = (int) D('Article')->sum('zan');
     ...
     //查询家政
	$counts['houseworksetting'] = (int) D('Houseworksetting')->count();
    $counts['totay_jiazheng'] = (int) D('Housework')->where(array('create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time))))->count();
    $counts['totay_jiazheng_queren'] = (int) D('Housework')->where(array('is_real' => 0))->count();
    $counts['lifeservice_dianping'] = (int) D('Lifeservicedianping')->count();   
    $counts['totay_lifeservice_dianping'] = (int) D('Lifeservicedianping')->where(array('create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time))))->count();
    ...
    $counts['totay_bill'] = (int) D('Billorder')->where(array('create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time))))->count();
